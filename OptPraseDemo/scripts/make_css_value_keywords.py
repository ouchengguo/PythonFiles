...
class CSSValueKeywordsWriter(in_generator.Writer):
    ...
    def generate_header(self):
        enum_enties = list(map(self._enum_declaration, [{'enum_name': 'CSSValueInvalid', 'enum_value': 0}] + self._value_keywords))
        return HEADER_TEMPLATE % {
            'license': license.license_for_generated_cpp(),
            'class_name': self.class_name,
            'value_keyword_enums': "\n".join(enum_enties),
            'value_keywords_count': len(enum_enties),
            'max_value_keyword_length': reduce(max, list(map(len, [property['name'] for property in self._value_keywords]))),
        }

    def _value_keywords_with_mode(self, mode):
        return [property for property in self._value_keywords if property['mode'] == mode]
    ...
    def generate_implementation(self):
        ...
        gperf_input = GPERF_TEMPLATE % {
            'license': license.license_for_generated_cpp(),
            'class_name': self.class_name,
            'value_keyword_strings': '\n'.join(['    "%(name)s\\0"' % property for property in self._value_keywords]),
            'value_keyword_offsets': '\n'.join(['  %d,' % offset for offset in keyword_offsets]),
            'value_keyword_to_enum_map': '\n'.join(['%(name)s, %(enum_name)s' % property for property in self._value_keywords]),
